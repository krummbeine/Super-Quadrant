 ...
 * @version 1.2
 */
public class MutterMine extends Element
{
     ...
     */
    public MutterMine(Point startPosition, int typ, float geschwindigkeit, Kamera kamera, boolean mitKameraVerfolgen, boolean sollGespeichertWerden) {
        super(
    ...
    /**
     * Da sich das MutterMine bewegt, muss zum Speichern die StartPosition extra gesichert werden !!
     * Ansonsten wird das MutterMine nach dem Laden an einer anderen Position erstellt
    ...
    /**
     * Aktualisiert und animiert das MutterMine
    ...
    protected void beruehrung(){
        ...
        if(warten > 0){
            if(warten % 50 == 0){
                Element.hinzufuegen(
                        new KindMine(
                                new Point((int)(x + random.nextInt(Fenster.GITTERGROESSE) - Fenster.GITTERGROESSE / 2),
                                        (int)(elementQuadrant.y - Fenster.GITTERGROESSE)), typ,
                                random.nextInt(40) / 100f, kamera, false, false
                        )
                );
            }
            warten--;
